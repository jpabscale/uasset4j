...
// Source: UAssetAPI/UAssetAPI/PropertyTypes/Structs/Movies/MovieSceneFloatChannelPropertyData.cs
...
class MovieSceneFloatChannelPropertyData : PropertyData {
    var Value: FMovieSceneFloatChannel?
        get() = GetObject<FMovieSceneFloatChannel>()
        set(v) = SetObject(v)
    ...
    override fun Read(reader: AssetBinaryReader, includeHeader: Boolean, leng1: Long, leng2: Long, serializationContext: PropertySerializationContext) {
        ...
        Value = FMovieSceneFloatChannel(reader)
    }
    ...
    override fun Write(writer: AssetBinaryWriter, includeHeader: Boolean, serializationContext: PropertySerializationContext): Int {
        ...
        Value?.Write(writer) { writer.WriteSingle(it) }
        return writer.position - offset
    }

    override fun CreateClone(): PropertyData = MovieSceneFloatChannelPropertyData()

    constructor(name: FName?) : super(name)
    constructor() : super()

    companion object {
        private val CurrentPropertyType = FString("MovieSceneFloatChannel")
    }
}
