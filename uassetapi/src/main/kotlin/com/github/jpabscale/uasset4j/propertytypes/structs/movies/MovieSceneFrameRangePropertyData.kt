...
// Source: UAssetAPI/UAssetAPI/PropertyTypes/Structs/Movies/MovieSceneFrameRangePropertyData.cs
...
class MovieSceneFrameRangePropertyData : PropertyData {
    var Value: TRange<FFrameNumber>?
        get() = GetObject<TRange<FFrameNumber>>()
        set(v) = SetObject(v)
    ...
    override fun Read(reader: AssetBinaryReader, includeHeader: Boolean, leng1: Long, leng2: Long, serializationContext: PropertySerializationContext) {
        ...
        Value = TRange(reader) { FFrameNumber(reader) }
    }
    ...
    override fun Write(writer: AssetBinaryWriter, includeHeader: Boolean, serializationContext: PropertySerializationContext): Int {
        ...
        Value?.Write(writer) { it.Write(writer) }
        return writer.position - offset
    }

    override fun CreateClone(): PropertyData = MovieSceneFrameRangePropertyData()

    constructor(name: FName?) : super(name)
    constructor() : super()

    companion object {
        private val CurrentPropertyType = FString("MovieSceneFrameRange")
    }
}
